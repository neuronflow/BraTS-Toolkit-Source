...
docker run --rm -d --name=greedy_elephant -p 5000:5000 -p 9181:9181 --user $(id -u):$(id -g) -v "$2":"/data/import/dicom_import" -v "$3":"/data/export/nifti_export" -v "$4":"/data/import/exam_import" -v "$5":"/data/export/exam_export" projectelephant/server redis-server
