...
def start_docker(exam_import_folder=None,
    ...
    if operatingSystem == "Windows":
        bashscript = './backend_scripts/win_docker.cmd'
    else:
        if mode == "cpu":
            bashscript = './backend_scripts/unix_docker.sh'
        elif mode == "robex":
            bashscript = './backend_scripts/unix_docker.sh'
        elif mode == "gpu":
            bashscript = './backend_scripts/unix_docker_gpu.sh'
        elif mode == "gpu_hdbet":
            bashscript = './backend_scripts/unix_docker_gpu.sh'

    # generate subprocess call
    readableCmd = bashscript, "3", dicom_import_folder, nifti_export_folder, exam_import_folder, exam_export_folder
